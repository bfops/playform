...
impl<'a> SurroundingsLoader<'a> {
  ...
  pub fn update(
    ...
    self.update_queues(position);
    self.load_some(gl, terrain_buffers, id_allocator, physics);
  }

  #[inline]
  fn update_queues(&mut self, block_position: BlockPosition) {
    if Some(block_position) != self.last_position {
      ...
      // TODO: iterating through want_loaded is slow. Do smarter diffs.
      ...
      match self.unload_queue.pop_front() {
        None =>
          match self.load_queue.pop_front() {
            None => break,
            Some(block_position) => {
              let block = self.terrain.load(id_allocator, &block_position);
