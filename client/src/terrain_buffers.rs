...
impl<'a> TerrainBuffers<'a> {
  ...
  /// Insert into VRAM data that is global to a given terrain block.
  pub fn push_block_data(
    ...
    let len = TEXTURE_LEN[lod.0 as usize];
  ...
  /// Remove from VRAM the data that is global to a given terrain block.
